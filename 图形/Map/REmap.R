...
## 颜色
##    可以使用颜色名(比如’red’,’skyblue’等),
##    RGB(“#1b1b1b”,“#fff”) 
##    或者一个rgba的形式(“rgba(100,100,100,1)”),可以在这里找到颜色对照表.
#####################################################################
# 2. remapC是用于创建分级统计图(Choropleth map).即根据子区域数值的多少进行深浅不同的颜色填充的地图形式.目前支持的地图为:
##   ‘china’ 中国省份地图
##   ‘world’ 世界地图
##   各省市地图,如’广东’,’西藏’等…
remapC(data,
...
mapNames(mapType = 'world')
## mapType :one of "china", "world" or the name of province in China
######################################################################
## 3.remapB是用于创建一个以百度地图为底图的recharts效果
...
###############################################################################
# markLine是Echarts中进行标线的工具.通过标线(直线,曲线),可以完成很多有意思的可视化。markLine相关的参数,这些参数可以在remapC或者remapB中调用:
##
##     markLineData 标线使用的数据,第一列为出发地,第二列为目的地
##     markLineTheme控制了标线的风格,使用markLineControl来调用
